...
struct ContentView: View {
    ...
    private static let defaultCities: [City] = [
        ...
        City(name: "Zürich",            latitude: 47.3775, longitude: 8.49540,   timeZoneID: "Europe/Zurich"),
        City(name: "Geneve",            latitude: 46.2040, longitude: 6.14300,  timeZoneID: "Europe/Zurich"),
        City(name: "Paris",             latitude: 48.8555, longitude: 2.34880,  timeZoneID: "Europe/Paris"),
        City(name: "Brussels",          latitude: 50.8524, longitude: 4.34180,  timeZoneID: "Europe/Brussels"),
        ...
        City(name: "Vancouver B.C.",    latitude: 49.2820, longitude: -123.120, timeZoneID: "Canada/Pacific"),
        ...
        City(name: "Barcelona",         latitude: 41.3860, longitude: 2.17300,  timeZoneID: "Europe/Madrid"),
    ...
    // Format local time in the given time zone, and append a UTC-style numeric offset label.
    // If minutes are 00, show "UTC±HH"; otherwise show "UTC±HH:MM" with zero-padded hours.
    func localTime(for date: Date, in timeZoneID: String) -> String {
        let tz = TimeZone(identifier: timeZoneID) ?? .gmt
        
        // 1) Local clock time in the city’s time zone
        let timeFormatter = DateFormatter()
        timeFormatter.locale = Locale(identifier: "en_US_POSIX")
        timeFormatter.timeZone = tz
        timeFormatter.dateFormat = "HH:mm:ss YYYY-MM-dd"
        let timeString = timeFormatter.string(from: date)
        
        // 2) Compute offset including DST at this date and render per your preference.
        ...
        if mm == 0 {
            // No minutes -> just UTC±HH (no zero-padding requested here)
            offsetString = "UTC\(sign)\(hh)"
        } else {
            // Include minutes and zero-pad hours and minutes -> UTC±HH:MM
            offsetString = String(format: "UTC%@%02d:%02d", sign, hh, mm)
        }
        ...
        ScrollView {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    case "Zürich":
                        Text(" Zürich:     \(localTime(for: now, in: "Europe/Zurich"))")
                    case "Geneve":
                        Text(" Geneve:   \(localTime(for: now, in: "Europe/Zurich"))")
                    case "Paris":
                        Text(" Paris:      \(localTime(for: now, in: "Europe/Paris"))")
                    case "Brussels":
                        Text(" Brussels:  \(localTime(for: now, in: "Europe/Brussels"))")
                    case "Anchorage Alaska":
                        Text(" Anchorage:  \(localTime(for: now, in: "America/Anchorage"))")
                    case "Vancouver B.C.":
                        Text(" Vancouver: \(localTime(for: now, in: "Canada/Pacific"))")
                    case "Washington D.C.":
